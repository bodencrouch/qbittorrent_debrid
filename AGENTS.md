...
- `qbx/web/matcher/`: React Control Shell source (Vite); built assets in `qbx/web/matcher/dist/` are served at `/`.
...
- `packaging/`: systemd units, env example, and provisional YAML defaults.
- `Dockerfile` and `docker-compose.yml`: containerized runtime and local deployment.
- `install.sh`: convenience installer that creates and uses a local `.venv`.
...
- `cd qbx/web/matcher && npm install && npm run build`: build the Control Shell SPA served at `/`.
